...
public class AdminBizImpl implements AdminBiz {
  ...
  public ReturnT<String> processCallback(List<HandleProcessCallbackParam> callbackParamList) {
    for (HandleProcessCallbackParam handleProcessCallbackParam : callbackParamList) {
      ...
      logger.debug(
          ">>>>>>>>> JobApiController.processCallback {}, handleCallbackParam={},"
              + " callbackResult={}",
          (callbackResult.getCode() == IJobHandler.SUCCESS.getCode() ? "success" : "fail"),
  ...
  private ReturnT<String> processCallback(HandleProcessCallbackParam handleProcessCallbackParam) {
    int result =
        jobLogMapper.updateProcessId(
            handleProcessCallbackParam.getLogId(), handleProcessCallbackParam.getProcessId());
  ...
  private ReturnT<String> callback(HandleCallbackParam handleCallbackParam) {
    ...
    if (!StringUtils.isEmpty(log.getProcessId())
        && IJobHandler.FAIL_TIMEOUT.getCode() == resultCode) {
  ...
  public ReturnT<String> registry(RegistryParam registryParam) {
    ...
    int ret =
        jobRegistryMapper.registryUpdate(
            registryParam.getRegistryGroup(),
            registryParam.getRegistryKey(),
            registryParam.getRegistryValue(),
            registryParam.getCpuUsage(),
            registryParam.getMemoryUsage(),
            registryParam.getLoadAverage(),
            new Date());
    if (ret < 1) {
      jobRegistryMapper.registrySave(
          registryParam.getRegistryGroup(),
          registryParam.getRegistryKey(),
          registryParam.getRegistryValue(),
          registryParam.getCpuUsage(),
